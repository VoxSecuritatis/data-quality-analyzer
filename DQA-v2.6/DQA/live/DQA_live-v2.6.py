...
try:
    ...
    # ----------- Export: Schema YAML -----------
    schema_filename = f"{timestamp}-{input_filename}.schema.expected.yaml"
    schema_path = os.path.join(dst_folder, schema_filename)
    try:
        dqa.export_schema_yaml(df, output_path=schema_path)
        print(f"[SCHEMA] Schema export successful.")
        print(f"[SCHEMA] Saved to: {schema_path}")
    except Exception as e:
        print(f"[ERROR] Failed to export schema YAML.\nReason: {e}")
